...
### Features
- Complete debug workflow with tool wait and instance create.
...
- Infer Mutation, CreatesResource, and RequiresAuth from Effects in schema output.
- Filter qcs tags when cloning tools.
- Resolve unknown commit and built values for `go install @tag`.
- Propagate remote shell exits cleanly from `agr instance login`.
- Render Code, Message, and RequestId uniformly in text errors.
- Hint the positional form on unknown flags for instance file upload and download.
...
- Apply `--jq` to the Data field instead of the full envelope.
- Resolve unknown commit and built values in `agr version` output.
